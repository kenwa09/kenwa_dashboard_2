...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const shares2 = computed(() => data.value?.shares || []);
    ...
    const filteredShares = computed(() => {
      ...
      if (search.value.trim()) {
        ...
        result = result.filter((s) => {
          ...
          return owner?.name?.toLowerCase().includes(q) || target?.name?.toLowerCase().includes(q) || loc?.label?.toLowerCase().includes(q) || s.targetContactName?.toLowerCase()?.includes(q);
        });
      }
      return result;
    });
...
//# sourceMappingURL=shares-Cfg3rpQH.js.map

    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderTeleport(_push, (_push2) => {
        if (unref(selectedOrder)) {
          ...
          if (unref(selectedOrder).items?.length) {
            ...
            ssrRenderList(unref(selectedOrder).items, (item) => {
              _push2(`<div class="order-item" data-v-cfb0cbcb><span class="item-qty" data-v-cfb0cbcb>${ssrInterpolate(item.quantity)}×</span><span class="item-name" data-v-cfb0cbcb>${ssrInterpolate(item.productName)}</span><span class="item-price" data-v-cfb0cbcb>€${ssrInterpolate((item.price * item.quantity).toFixed(2))}</span></div>`);
            });
...
//# sourceMappingURL=index-Dpjjx7Ar.js.map
